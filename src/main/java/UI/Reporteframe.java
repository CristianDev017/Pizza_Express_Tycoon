...
public class Reporteframe extends JFrame {
    ...
    public Reporteframe(int idSucursal, String nombreSucursal) {
        this.idSucursal = idSucursal;

        setTitle("Reportes - " + nombreSucursal);
        setSize(900, 500);
        setLocationRelativeTo(null);
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setLayout(new BorderLayout());

        JLabel lblTitulo = new JLabel("Reporte de Partidas - " + nombreSucursal, SwingConstants.LEFT);
        lblTitulo.setFont(new Font("Arial", Font.BOLD, 20));
        lblTitulo.setForeground(new Color(200, 70, 0));
        lblTitulo.setBorder(new EmptyBorder(20, 25, 15, 25));
        add(lblTitulo, BorderLayout.NORTH);

        String[] columnas = {"Jugador", "Puntaje", "Nivel Alcanzado", "Pedidos Completados", "Cancelados", "No Entregados", "Fecha"};
        ...
        datos = service.obtenerReporteSucursal(idSucursal);
    ...
    private void exportarCSV() {
        ...
        if (resultado == JFileChooser.APPROVE_OPTION) {
            ...
            Reporteservice service = new Reporteservice();
            boolean exito = service.exportarCSV(datos, ruta);
